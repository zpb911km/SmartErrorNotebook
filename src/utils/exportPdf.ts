...
export async function exportQuestionsToPDF(
  ...
  try {
    ...
    container.style.cssText =
      'position:fixed;top:0;left:0;width:800px;z-index:-1000;pointer-events:none'
  ...
  <div style="text-align:center;font-size:11px;color:#999;padding:10px 0">由 SmartErrorNotebook 生成</div>
    ...
    for (let p = 0; p < total; p++) {
      if (p > 0) pdf.addPage()
      const sy = Math.round(p * pagePx),
        sh = Math.min(Math.round(pagePx), canvas.height - sy)
