...
当前版本不支持远程同步。同步页只显示停用提示，不接受服务器地址或授权码。
仓库中的 `server/` 是与当前数据库模型不兼容的历史实现，请勿部署或连接。
跨设备迁移请使用 JSON 导出和导入。未来实现必须满足
[同步协议草案](SYNC_PROTOCOL.md) 中的关系和版本约束。

---

## 🌐 错题社区

当前版本不支持错题社区。社区入口和远程分享操作均已停用。
